...
const initDefaultMenu = async () => {
  try {
    ...
    let defaultMenu = await DefaultMenu.getDefaultMenu();

    // Update with seed catalog
    defaultMenu.categories = seedCatalog;
    defaultMenu.version = (defaultMenu.version || 0) + 1;
    await defaultMenu.save();

    console.log(`✅ Default menu initialized with ${seedCatalog.length} categories`);
    console.log(`   Total items: ${seedCatalog.reduce((sum, cat) => sum + cat.items.length, 0)}`);
    console.log("🎉 Default menu initialization completed.");
    process.exit(0);
  } catch (err) {
    console.error("❌ Default menu initialization failed", err);
    process.exit(1);
  }
};
