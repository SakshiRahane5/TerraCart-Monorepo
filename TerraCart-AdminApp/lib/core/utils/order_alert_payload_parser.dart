...
class OrderAlertPayloadParser {
  ...
  static String buildDedupeKey({
    ...
    final normalizedEvent = normalizeEventName(eventName);
    final entityId = extractEntityId(payload);
    if (entityId != null && entityId.isNotEmpty) {
      // Keep dedupe scoped per event type to avoid suppressing a payment alert
      // right after an order event for the same orderId.
      return '$normalizedEvent:entity:$entityId';
    }

    final cartId = extractCartId(payload);
    if (cartId != null && cartId.isNotEmpty) {
      return '$normalizedEvent:$cartId';
    }
