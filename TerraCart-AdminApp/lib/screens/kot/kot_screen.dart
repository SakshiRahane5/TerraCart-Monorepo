...
class _KotScreenState extends State<KotScreen> {
  ...
  final List<String> _statusFilters = ['All', 'pending', 'preparing', 'ready'];
  final List<String> _dateRanges = ['today', 'week', 'month', 'all'];
  ...
  Future<void> _loadKOTs() async {
    ...
    try {
      final role = Provider.of<AppProvider>(context, listen: false)
          .userRole
          .toLowerCase();
      final clearBeforeMs = role == 'cook'
          ? await _kotService.getCookKotClearBeforeMs()
          : null;
      ...
      if (mounted) {
        setState(() {
          var normalized = List<Map<String, dynamic>>.from(kots);
          if (clearBeforeMs != null && clearBeforeMs > 0) {
            normalized = normalized.where((item) {
              final createdAt =
                  _parseKotDate(item['createdAt'] ?? item['timestamp']);
              if (createdAt == null) return false;
              return createdAt.millisecondsSinceEpoch > clearBeforeMs;
            }).toList(growable: false);
          }
          normalized.sort((a, b) {
            final aTime =
                _parseKotDate(a['createdAt'] ?? a['timestamp']) ??
                    DateTime.fromMillisecondsSinceEpoch(0);
            final bTime =
                _parseKotDate(b['createdAt'] ?? b['timestamp']) ??
                    DateTime.fromMillisecondsSinceEpoch(0);
            return bTime.compareTo(aTime);
          });
          _kotItems = normalized;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  DateTime? _parseKotDate(dynamic value) {
  ...
  bool _matchesDateRange(Map<String, dynamic> item) {
    final range = _selectedDateRange.trim().toLowerCase();
    ...
    final today = _normalizeIstDateOnly(DateTime.now());
    final createdDay = _normalizeIstDateOnly(createdAt);
    ...
    if (range == 'month') {
      return createdDay.year == today.year && createdDay.month == today.month;
    }
    return true;
  }

  List<Map<String, dynamic>> get _dateFilteredItems {
    return _kotItems.where(_matchesDateRange).toList(growable: false);
  }

  List<Map<String, dynamic>> get _filteredItems {
    if (_selectedStatusFilter == 'All') return _dateFilteredItems;
    return _dateFilteredItems.where((item) {
      final rawStatus =
          (item['orderStatus'] ?? item['status'] ?? '').toString();
      return _statusKey(rawStatus) == _selectedStatusFilter.toLowerCase();
    }).toList();
  }
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 16),
                            Text(
                              'No KOTs found for selected filter',
  ...
  Widget _buildKotCard(Map<String, dynamic> kot) {
    ...
    final createdAtTime = _parseKotDate(createdAt);
    final timeAgo =
        createdAtTime != null ? DateTimeUtils.getTimeAgo(createdAtTime) : 'Recently';
