...
class _WaiterDashboardState extends State<WaiterDashboard> {
  ...
  void _setupSocketListeners() {
    ...
    void refreshAttendance(dynamic _) {
      if (!mounted) return;
    ...
    // Attendance events for realtime status updates on waiter dashboard
    _socketService.on('attendance:checked_in', refreshAttendance,
        debounce: true);
    _socketService.on('attendance:checked_out', refreshAttendance,
        debounce: true);
    _socketService.on('attendance:updated', refreshAttendance, debounce: true);
    _socketService.on('attendance:break_started', refreshAttendance,
        debounce: true);
    _socketService.on('attendance:break_ended', refreshAttendance,
        debounce: true);
  }
  ...
  Future<void> _loadDashboardData({bool showLoading = true}) async {
    ...
    try {
      final results = await Future.wait([
        ...
        _orderService
            .getOrders(status: null, limit: 50, includeHistory: true)
      ...
      if (mounted) {
        setState(() {
          ...
          final activeOrderPool = allOrders
              .where(_isManagerStyleActiveOrder)
              .toList(growable: false);

          final activeOrdersFromStats =
              (_stats['activeOrders'] as num?)?.toInt();
          _activeOrdersCount = activeOrdersFromStats ?? activeOrderPool.length;
          _activeOrders = activeOrderPool
              .where((o) => OrderStatusUtils.isDineInServiceType(o.serviceType))
              .take(5)
              .toList(growable: false);
          _isLoading = false;
        });
  ...
  bool _isManagerStyleActiveOrder(OrderModel order) {
    final normalizedStatus = OrderStatusUtils.normalizeStatus(order.status);
    return normalizedStatus == OrderStatusUtils.statusNew ||
        normalizedStatus == OrderStatusUtils.statusPreparing ||
        normalizedStatus == OrderStatusUtils.statusReady;
  }
