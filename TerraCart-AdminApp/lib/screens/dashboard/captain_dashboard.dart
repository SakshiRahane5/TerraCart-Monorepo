...
class _CaptainDashboardState extends State<CaptainDashboard> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _activeOrdersCount = (_stats['activeOrders'] as num?)?.toInt() ?? 0;
          _isLoading = false;
        });
  ...
  Future<void> _loadOrders() async {
    try {
      ...
      if (mounted) {
        setState(() {
          // Match manager dashboard "active orders" definition.
          _currentOrders = orders.where(_isManagerStyleActiveOrder).toList();
