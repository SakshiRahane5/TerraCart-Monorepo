...
class _ManagerEmployeeAttendanceScreenState
  ...
  // Kept dynamic-compatible to safely handle hot-reload state where older
  // map shape might still be Map<String, List<Map<...>>>.
  ...
  void _setupSocketListeners() {
    _socketService.on('attendance:checked_in', _onAttendanceCheckedIn,
        debounce: true, delay: const Duration(milliseconds: 500));
    _socketService.on('attendance:checked_out', _onAttendanceCheckedOut,
        debounce: true, delay: const Duration(milliseconds: 500));
    _socketService.on('attendance:updated', _onAttendanceUpdated,
        debounce: true, delay: const Duration(milliseconds: 500));
    _socketService.on('attendance:deleted', _onAttendanceDeleted,
        debounce: true, delay: const Duration(milliseconds: 500));
  }

  void _removeSocketListeners() {
    _socketService.off('attendance:checked_in', _onAttendanceCheckedIn);
    _socketService.off('attendance:checked_out', _onAttendanceCheckedOut);
    _socketService.off('attendance:updated', _onAttendanceUpdated);
    _socketService.off('attendance:deleted', _onAttendanceDeleted);
  }

  void _handleRealtimeAttendanceChange() {
    if (!mounted) return;
    CacheService().remove(CacheService.todayAttendance);
    _loadData();
  }

  void _onAttendanceCheckedIn(dynamic _) => _handleRealtimeAttendanceChange();
  void _onAttendanceCheckedOut(dynamic _) => _handleRealtimeAttendanceChange();
  void _onAttendanceUpdated(dynamic _) => _handleRealtimeAttendanceChange();
  void _onAttendanceDeleted(dynamic _) => _handleRealtimeAttendanceChange();
  ...
  String _normalizeAttendanceStatus(Map<String, dynamic> attendance) {
    final explicitStatus =
        attendance['attendanceStatus']?.toString().toLowerCase().trim();
    if (explicitStatus != null && explicitStatus.isNotEmpty) {
      return explicitStatus;
    }

    final checkInStatus =
        attendance['checkInStatus']?.toString().toLowerCase().trim();
    if (checkInStatus != null && checkInStatus.isNotEmpty) {
      if (checkInStatus == 'checked_in' && attendance['isOnBreak'] == true) {
        return 'on_break';
      }
    ...
    if (checkOut != null || attendance['isCheckedOut'] == true) {
      return 'checked_out';
    }
    ...
    if (status == 'absent' || status == 'on_leave' || status == 'sick') {
      return 'absent';
    }
  ...
  int _attendancePriority(Map<String, dynamic> attendance) {
    final status = _normalizeAttendanceStatus(attendance);
    switch (status) {
      case 'checked_out':
        return 4;
      case 'on_break':
  ...
  bool _shouldReplaceAttendanceRecord(
    Map<String, dynamic> candidate,
    Map<String, dynamic> current,
  ) {
    final candidatePriority = _attendancePriority(candidate);
    final currentPriority = _attendancePriority(current);
    if (candidatePriority != currentPriority) {
      return candidatePriority > currentPriority;
    }

    final candidateTime = _parseAttendanceDate(
      candidate['updatedAt'] ??
    ...
    final currentTime = _parseAttendanceDate(
      current['updatedAt'] ??
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          // Keep one best attendance record per employee.
          ...
          for (var att in attendanceList) {
            ...
            if (employeeId != null) {
              final empId = employeeId is Map
                  ...
                  : employeeId.toString();
              if (empId != null) {
                final normalizedRecord = Map<String, dynamic>.from(att);
                final current = _todayAttendanceMap[empId];
                if (current == null ||
                    _shouldReplaceAttendanceRecord(normalizedRecord, current)) {
                  _todayAttendanceMap[empId] = normalizedRecord;
                }
              }
            }
          }
  ...
  Map<String, dynamic>? _getTodayAttendance(String employeeId) {
    final value = _todayAttendanceMap[employeeId];
    if (value is Map) {
    ...
    if (value is List) {
      ...
      for (final entry in value) {
        ...
        if (selected == null ||
            _shouldReplaceAttendanceRecord(candidate, selected)) {
          selected = candidate;
        }
      }
      return selected;
  ...
  String _getStatusText(Map<String, dynamic>? attendance) {
    ...
    final normalizedStatus = _normalizeAttendanceStatus(attendance);
  ...
  bool _isCheckedOut(Map<String, dynamic>? attendance) {
    if (attendance == null) return false;
    return _normalizeAttendanceStatus(attendance) == 'checked_out';
  }

  bool _isOnBreak(Map<String, dynamic>? attendance) {
    if (attendance == null) return false;
    return _normalizeAttendanceStatus(attendance) == 'on_break';
  }
  ...
  void _showManualCheckInDialog() {
    final notCheckedIn = _filteredEmployees.where((e) {
